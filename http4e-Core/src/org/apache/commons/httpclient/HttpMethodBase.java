 ...
 * $Revision: 429849 $
 * $Date: 2006-08-08 22:06:09 +0000 (Tue, 08 Aug 2006) $
 *
 * ====================================================================
 *
 ...
 * <ul>
 * <li>{@link #addRequestHeaders addRequestHeaders(HttpState,HttpConnection)} to write those headers
 * </li>
 * </ul>
 * </p>
 *
 ...
 * <ul>
 * <li>{@link #processResponseHeaders processResponseHeaders(HttpState,HttpConnection)} to handle
 * those headers</li>
 * </ul>
 * </p>
 *
 ...
 *
 * @version $Revision: 429849 $ $Date: 2006-08-08 22:06:09 +0000 (Tue, 08 Aug 2006) $
 */
public abstract class HttpMethodBase implements HttpMethod {
    ...
    /**
     * Constructor specifying a URI. It is responsibility of the caller to ensure that URI elements
    ...
    @Override
    public ApacheHttpListener getApacheHttpListener() {
        return apacheHttpListener;
    }

    @Override
    public void setApacheHttpListener(final ApacheHttpListener apacheHttpListener) {
    ...
    /**
     * Obtains the name of the HTTP method as used in the HTTP request line, for example <tt>"GET"</tt>
     * or <tt>"POST"</tt>.
     * 
    ...
    /**
     * Sets whether or not the HTTP method should automatically follow HTTP redirects (status code 302,
     * etc.)
     * 
    ...
    /**
     * Returns <tt>true</tt> if the HTTP method should automatically follow HTTP redirects (status code
     * 302, etc.), <tt>false</tt> otherwise.
     * 
     * @return <tt>true</tt> if the method will automatically follow HTTP redirects, <tt>false</tt>
     *         otherwise.
     */
    @Override
    ...
    /**
     * Returns <tt>true</tt> if the HTTP method should automatically handle HTTP authentication
     * challenges (status code 401, etc.), <tt>false</tt> otherwise
     *
     * @return <tt>true</tt> if authentication challenges will be processed automatically,
     *         <tt>false</tt> otherwise.
     * 
     * @since 2.0
     */
    ...
    /**
     * Returns <tt>true</tt> if version 1.1 of the HTTP protocol should be used per default,
     * <tt>false</tt> if version 1.0 should be used.
     *
    ...
    /**
     * Sets the path of the HTTP method. It is responsibility of the caller to ensure that the path is
    ...
    /**
     * Adds the specified request header, NOT overwriting any previous value. Note that header-name
    ...
    /**
     * Gets the path of this HTTP method. Calling this method <em>after</em> the request has been
     * executed will return the <em>actual</em> path, following any redirects automatically handled by
     * this HTTP method.
     *
    ...
    /**
     * Sets the query string of this HTTP method. The caller must ensure that the string is properly URL
     * encoded. The query string should not start with the question mark character.
     *
    ...
    /**
     * Sets the query string of this HTTP method. The pairs are encoded as UTF-8 characters. To use a
     * different charset the parameters can be encoded manually using EncodingUtil and set as a single
     * String.
     *
     * @param params an array of {@link NameValuePair}s to add as query string parameters. The
     *               name/value pairs will be automcatically URL encoded
     * 
     * @see EncodingUtil#formUrlEncode(NameValuePair[], String)
    ...
    /**
     * Set the specified request header, overwriting any previous value. Note that header-name matching
     * is case-insensitive.
     *
    ...
    /**
     * Sets the specified request header, overwriting any previous value. Note that header-name matching
     * is case insensitive.
     * 
    ...
    public void setRequestHeader(final Header header) {
        ...
        for (final Header header2 : headers) {
    ...
    /**
     * Returns the specified request header. Note that header-name matching is case insensitive.
     * <tt>null</tt> will be returned if either <i>headerName</i> is <tt>null</tt> or there is no
     * matching header for <i>headerName</i>.
     * 
    ...
    /**
     * Gets the {@link HeaderGroup header group} storing the response trailer headers as per RFC 2616
     * section 3.6.1.
     * 
     * @return a HeaderGroup
     * 
    ...
    /**
     * Returns an array of the response headers that the HTTP method currently has in the order in which
     * they were read.
     *
    ...
    /**
     * Gets the response header associated with the given name. Header name matching is case
     * insensitive. <tt>null</tt> will be returned if either <i>headerName</i> is <tt>null</tt> or there
     * is no matching header for <i>headerName</i>.
     *
    ...
    /**
     * Return the length (in bytes) of the response body, as specified in a <tt>Content-Length</tt>
     * header.
     *
     * <p>
     ...
     * </p>
     *
     * @return content length, if <tt>Content-Length</tt> header is available. <tt>0</tt> indicates that
     *         the request has no body. If <tt>Content-Length</tt> header is not present, the method
     *         returns <tt>-1</tt>.
     */
    ...
    /**
     * Returns the response body of the HTTP method, if any, as an array of bytes. If response body is
     * not available or cannot be read, returns <tt>null</tt>
     * 
     * Note: This will cause the entire response body to be buffered in memory. A malicious server may
     * easily exhaust all the VM memory. It is strongly recommended, to use getResponseAsStream if the
    ...
    public byte[] getResponseBody() throws IOException {
        if (responseBody == null) {
            ...
            if (instream != null) {
                ...
                final ByteArrayOutputStream outstream = new ByteArrayOutputStream(contentLength > 0 ? (int) contentLength : DEFAULT_INITIAL_BUFFER_SIZE);
    ...
    /**
     * Returns the response body of the HTTP method, if any, as an {@link InputStream}. If response body
     * is not available, returns <tt>null</tt>
     * 
     * @return The response body
     * 
     * @throws IOException If an I/O (transport) problem occurs while obtaining the response body.
    ...
    /**
     * Returns the response body of the HTTP method, if any, as a {@link String}. If response body is
     * not available or cannot be read, returns <tt>null</tt> The string conversion on the data is done
     * using the character encoding specified in <tt>Content-Type</tt> header.
     * 
     * Note: This will cause the entire response body to be buffered in memory. A malicious server may
     * easily exhaust all the VM memory. It is strongly recommended, to use getResponseAsStream if the
     * content length of the response is unknown or resonably large.
    ...
    /**
     * Returns an array of the response footers that the HTTP method currently has in the order in which
     * they were read.
     *
    ...
    /**
     * Gets the response footer associated with the given name. Footer name matching is case
     * insensitive. <tt>null</tt> will be returned if either <i>footerName</i> is <tt>null</tt> or there
     * is no matching footer for <i>footerName</i> or there are no footers available. If there are
     * multiple footers with the same name, there values will be combined with the ',' separator as
    ...
    /**
     * Returns a stream from which the body of the current response may be read. If the method has not
     * yet been executed, if <code>responseBodyConsumed</code> has been called, or if the stream
     * returned by a previous call has been closed, <code>null</code> will be returned.
     *
    ...
    /**
     * Defines how strictly HttpClient follows the HTTP protocol specification (RFC 2616 and other
     * relevant RFCs). In the strict mode HttpClient precisely implements the requirements of the
     * specification, whereas in non-strict mode it attempts to mimic the exact behaviour of commonly
    ...
    /**
     * Sets whether or not the connection should be force-closed when no longer needed. This value
     * should only be set to <code>true</code> in abnormal circumstances, such as HTTP protocol
     * violations.
     * 
    ...
    /**
     * Tests if the connection should be closed after the method has been executed. The connection will
     * be left open when using HTTP/1.1 or if <tt>Connection:
     ...
     *
     * @param state {@link HttpState state} information to associate with this request. Must be
     *              non-null.
     * @param conn  the {@link HttpConnection connection} to used to execute this HTTP method. Must be
     *              non-null.
     *
    ...
    /**
     * Returns <tt>true</tt> if the HTTP method has been already {@link #execute executed}, but not
     * {@link #recycle recycled}.
     * 
    ...
    /**
     * Recycles the HTTP method so that it can be used again. Note that all of the instance variables
     * will be reset once this method has been called. This method will also release the connection
     * being used by this HTTP method.
     * 
    ...
    /**
     * Releases the connection being used by this HTTP method. In particular the connection is used to
     * read the response(if there is one) and will be held until the response has been read. If the
     * connection can be reused by other HTTP methods it is NOT closed at this point.
     *
     * @since 2.0
     */
    @Override
    ...
    /**
     * Remove the request header associated with the given name. Note that header-name matching is case
     * insensitive.
     *
    ...
    private CookieSpec getCookieSpec(final HttpState state) {
        if (cookiespec == null) {
            ...
            cookiespec.setValidDateFormats((Collection) params.getParameter(HttpMethodParams.DATE_PATTERNS));
        }
        return cookiespec;
    }

    /**
     * Generates <tt>Cookie</tt> request headers for those {@link Cookie cookie}s that match the given
     * host, port and path.
     *
     * @param state the {@link HttpState state} information associated with this method
    ...
    protected void addCookieRequestHeader(final HttpState state, final HttpConnection conn) throws IOException, HttpException {
        ...
        final CookieSpec matcher = getCookieSpec(state);
        ...
        if (cookies != null && cookies.length > 0) {
            if (getParams().isParameterTrue(HttpMethodParams.SINGLE_COOKIE_HEADER)) {
                ...
                for (final Cookie element : cookies) {
                    final String s = matcher.formatCookie(element);
                    getRequestHeaderGroup().addHeader(new Header("Cookie", s, true));
                }
            }
            if (matcher instanceof final CookieVersionSupport versupport) {
    ...
    /**
     * Generates <tt>Host</tt> request header, as long as no <tt>Host</tt> request header already
     * exists.
     *
     * @param state the {@link HttpState state} information associated with this method
     * @param conn  the {@link HttpConnection connection} used to execute this HTTP method
    ...
    /**
     * Generates <tt>Proxy-Connection: Keep-Alive</tt> request header when communicating via a proxy
     * server.
     *
     * @param state the {@link HttpState state} information associated with this method
     * @param conn  the {@link HttpConnection connection} used to execute this HTTP method
    ...
    /**
     * Generates all the required request {@link Header header}s to be submitted via the given
     * {@link HttpConnection connection}.
     *
     * <p>
     * This implementation adds <tt>User-Agent</tt>, <tt>Host</tt>, <tt>Cookie</tt>,
     * <tt>Authorization</tt>, <tt>Proxy-Authorization</tt> and <tt>Proxy-Connection</tt> headers, when
     * appropriate.
     * </p>
     *
     * <p>
     * Subclasses may want to override this method to to add additional headers, and may choose to
     * invoke this implementation (via <tt>super</tt>) to add the "standard" headers.
     * </p>
     *
     * @param state the {@link HttpState state} information associated with this method
     * @param conn  the {@link HttpConnection connection} used to execute this HTTP method
    ...
    /**
     * Generates default <tt>User-Agent</tt> request header, as long as no <tt>User-Agent</tt> request
    ...
    /**
     * Throws an {@link IllegalStateException} if the HTTP method has been already {@link #execute
    ...
    /**
     * Throws an {@link IllegalStateException} if the HTTP method has not been {@link #execute executed}
     * since last {@link #recycle recycle}.
     *
     *
     ...
     */
    protected static String generateRequestLine(final HttpConnection connection, final String name, final String requestPath, final String query,
    ...
    /**
     * This method is invoked immediately after {@link #readResponseBody(HttpState,HttpConnection)} and
     * can be overridden by sub-classes in order to provide custom body processing.
     *
     * <p>
    ...
    /**
     * This method is invoked immediately after {@link #readResponseHeaders(HttpState,HttpConnection)}
     ...
     * adding the relevant cookies to the given {@link HttpState}.
     * </p>
     *
     * @param state the {@link HttpState state} information associated with this method
     * @param conn  the {@link HttpConnection connection} used to execute this HTTP method
    ...
    protected void processResponseHeaders(final HttpState state, final HttpConnection conn) {
        ...
        final CookieSpec parser = getCookieSpec(state);
        ...
        if (parser instanceof final CookieVersionSupport versupport) {
    ...
    /**
     * This method processes the specified cookie headers. It is invoked from within
     * {@link #processResponseHeaders(HttpState,HttpConnection)}
     *
     * @param headers cookie {@link Header}s to be processed
     * @param state   the {@link HttpState state} information associated with this HTTP method
     * @param conn    the {@link HttpConnection connection} used to execute this HTTP method
     */
    ...
    /**
     * This method is invoked immediately after {@link #readStatusLine(HttpState,HttpConnection)} and
     * can be overridden by sub-classes in order to provide custom response status line processing.
     *
     * @param state the {@link HttpState state} information associated with this method
     * @param conn  the {@link HttpConnection connection} used to execute this HTTP method
     *
     ...
     *
     * <ol>
     * <li>{@link #readStatusLine(HttpState,HttpConnection)} is invoked to read the request line.</li>
     * <li>{@link #processStatusLine(HttpState,HttpConnection)} is invoked, allowing the method to
     * process the status line if desired.</li>
     * <li>{@link #readResponseHeaders(HttpState,HttpConnection)} is invoked to read the associated
     * headers.</li>
     * <li>{@link #processResponseHeaders(HttpState,HttpConnection)} is invoked, allowing the method to
     * process the headers if desired.</li>
     * <li>{@link #readResponseBody(HttpState,HttpConnection)} is invoked to read the associated body
     * (if any).</li>
     * <li>{@link #processResponseBody(HttpState,HttpConnection)} is invoked, allowing the method to
     * process the response body if desired.</li>
     * </ol>
     *
     * Subclasses may want to override one or more of the above methods to to customize the processing.
     * (Or they may choose to override this method if dramatically different processing is required.)
     * </p>
     *
     * @param state the {@link HttpState state} information associated with this method
     * @param conn  the {@link HttpConnection connection} used to execute this HTTP method
     ...
     *
     * <p>
     * The current implementation wraps the socket level stream with an appropriate stream for the type
     * of response (chunked, content-length, or auto-close). If there is no response body, the
     * connection associated with the request will be returned to the connection manager.
     * </p>
     *
     * <p>
    ...
    /**
     * Returns the response body as an {@link InputStream input stream} corresponding to the values of
     * the <tt>Content-Length</tt> and <tt>Transfer-Encoding</tt> headers. If no response body is
    ...
    private InputStream readResponseBody(final HttpConnection conn) throws HttpException, IOException {
        ...
        if (result != null) {
            ...
            result = new AutoCloseInputStream(result, this::responseBodyConsumed);
        }
     ...
     * the first, each separated by a comma." - HTTP/1.0 (4.3)
     * </p>
     *
     * @param state the {@link HttpState state} information associated with this method
     * @param conn  the {@link HttpConnection connection} used to execute this HTTP method
    ...
    protected void readResponseHeaders(final HttpState state, final HttpConnection conn) throws IOException, HttpException {
        ...
        final Header[] headers = HttpParser.parseHeaders(conn.getResponseInputStream(), getParams().getHttpElementCharset());
    ...
    /**
     * Read the status line from the given {@link HttpConnection}, setting my {@link #getStatusCode
     * status code} and {@link #getStatusText status text}.
     *
     * <p>
     * Subclasses may want to override this method to to customize the processing.
     * </p>
     ...
     *
     * <ol>
     * <li>{@link #writeRequestLine(HttpState, HttpConnection)} is invoked to write the request line.
     * </li>
     * <li>{@link #writeRequestHeaders(HttpState, HttpConnection)} is invoked to write the associated
     * headers.</li>
     * <li><tt>\r\n</tt> is sent to close the head part of the request.</li>
     * <li>{@link #writeRequestBody(HttpState, HttpConnection)} is invoked to write the body part of the
     * request.</li>
     * </ol>
     *
     * <p>
    ...
    protected void writeRequest(final HttpState state, final HttpConnection conn) throws IOException, HttpException {
        ...
        conn.setApacheHttpListener(getApacheHttpListener());
     ...
     *
     * <p>
     * This method should return <tt>true</tt> if the request body was actually sent (or is empty), or
     * <tt>false</tt> if it could not be sent for some reason.
     * </p>
     *
     * <p>
     ...
     *
     * <p>
     * This implementation invokes {@link #addRequestHeaders(HttpState,HttpConnection)}, and then writes
    ...
    protected void writeRequestHeaders(final HttpState state, final HttpConnection conn) throws IOException, HttpException {
        ...
        for (final Header header : headers) {
            final String s = header.toExternalForm();
            if (Wire.HEADER_WIRE.enabled()) {
    ...
    private String getRequestLine(final HttpConnection conn) {
        return HttpMethodBase.generateRequestLine(conn, getName(), getPath(), getQueryString(), effectiveVersion.toString());
    }
    ...
    /**
     * Returns the HTTP version used with this method (may be <tt>null</tt> if undefined, that is, the
     ...
     *
     * @return <tt>true</tt> if the message may contain a body, <tt>false</tt> if it can not contain a
     *         message body
     */
    private static boolean canResponseHaveBody(final int status) {
        ...
        if (status >= 100 && status <= 199 || status == 204 || status == 304) { // NOT MODIFIED
            result = false;
        }
    ...
    /**
     * Returns proxy authentication realm, if it has been used during authentication process. Otherwise
     * returns <tt>null</tt>.
     * 
    ...
    /**
     * Returns authentication realm, if it has been used during authentication process. Otherwise
     * returns <tt>null</tt>.
     * 
     ...
     * 
     *             Returns the number of "recoverable" exceptions thrown and handled, to allow for
     ...
     *
     * <p>
     * The default behavior for this class is to check to see if the connection should be closed, and
     * close if need be, and to ensure that the connection is returned to the connection manager - if
     * and only if we are not still inside the execute call.
     * </p>
     *
     */
    ...
    /**
     * This method is a dirty hack intended to work around current (2.0) design flaw that prevents the
     * user from obtaining correct status code, headers and response body from the preceding HTTP
     * CONNECT method.
     * 
    ...
    /**
     * Returns <tt>true</tt> if the HTTP has been transmitted to the target server in its entirety,
     * <tt>false</tt> otherwise. This flag can be useful for recovery logic. If the request has not been
     * transmitted in its entirety, it is safe to retry the failed method.
     * 
