...
/**
 * A <code>SecureProtocolSocketFactory</code> that uses JSSE to create SSL sockets. It will also
 * support host name verification to help preventing man-in-the-middle attacks. Host name
 * verification is turned <b>on</b> by default but one will be able to turn it off, which might be a
 * useful feature during development. Host name verification will make sure the SSL sessions server
 * host name matches with the the host name returned in the server certificates "Common Name" field
...
public class StrictSSLProtocolSocketFactory implements SecureProtocolSocketFactory {
     ...
     *
     * @param verifyHostname The host name verification flag. If set to <code>true</code> the SSL
     *                       sessions server host name will be compared to the host name returned in the
     *                       server certificates "Common Name" field of the "SubjectDN" entry. If these
     *                       names do not match a Exception is thrown to indicate this. Enabling host
     *                       name verification will help to prevent from man-in-the-middle attacks. If
     *                       set to <code>false</code> host name verification is turned off.
     * 
     *                       Code sample:
     * 
     ...
     *
     */
    public StrictSSLProtocolSocketFactory(final boolean verifyHostname) {
        this.verifyHostname = verifyHostname;
    }

    /**
     * Constructor for StrictSSLProtocolSocketFactory. Host name verification will be enabled by
     * default.
     */
     ...
     *                       set to <code>false</code> host name verification is turned off.
     */
    public void setHostnameVerification(final boolean verifyHostname) {
        this.verifyHostname = verifyHostname;
    }
     ...
     *
     * @return Host name verification flag. Either <code>true</code> if host name verification is turned
     *         on, or <code>false</code> if host name verification is turned off.
     */
    ...
    public Socket createSocket(final String host, final int port, final InetAddress clientHost, final int clientPort) throws IOException, UnknownHostException {
        ...
        final SSLSocket sslSocket = (SSLSocket) sf.createSocket(host, port, clientHost, clientPort);
     ...
     * <p>
     * This method employs several techniques to circumvent the limitations of older JREs that do not
     * support connect timeout. When running in JRE 1.4 or above reflection is used to call
    ...
    public Socket createSocket(final String host, final int port) throws IOException, UnknownHostException {
        final SSLSocketFactory sf = (SSLSocketFactory) SSLSocketFactory.getDefault();
        final SSLSocket sslSocket = (SSLSocket) sf.createSocket(host, port);
        verifyHostname(sslSocket);
    ...
    public Socket createSocket(final Socket socket, final String host, final int port, final boolean autoClose) throws IOException, UnknownHostException {
        final SSLSocketFactory sf = (SSLSocketFactory) SSLSocketFactory.getDefault();
        final SSLSocket sslSocket = (SSLSocket) sf.createSocket(socket, host, port, autoClose);
        verifyHostname(sslSocket);
     ...
     * @exception SSLPeerUnverifiedException If there are problems obtaining the server certificates
     *                                       from the SSL session, or the server host name does not
     *                                       match with the "Common Name" in the server certificates
     *                                       SubjectDN.
     * @exception UnknownHostException       If we are not able to resolve the SSL sessions returned
     *                                       server host name.
     */
    private void verifyHostname(final SSLSocket socket) throws SSLPeerUnverifiedException, UnknownHostException {
        if (!verifyHostname) {
            return;
        }
        ...
        final X509Certificate[] certs = session.getPeerCertificateChain();
        if (certs == null || certs.length == 0) {
            throw new SSLPeerUnverifiedException("No server certificates found!");
        }

        // get the servers DN in its string representation
        final String dn = certs[0].getSubjectDN().getName();
    ...
    /**
     * Parses a X.500 distinguished name for the value of the "Common Name" field. This is done a bit
     * sloppy right now and should probably be done a bit more according to <code>RFC 2253</code>.
     *
    ...
    private String getCN(String dn) {
        ...
        final char[] dncs = dn.toCharArray();
    ...
    public boolean equals(final Object obj) {
        if (obj != null && obj.getClass().equals(StrictSSLProtocolSocketFactory.class)) {
            return ((StrictSSLProtocolSocketFactory) obj).getHostnameVerification() == verifyHostname;
        } else {
            return false;
        }
    }
