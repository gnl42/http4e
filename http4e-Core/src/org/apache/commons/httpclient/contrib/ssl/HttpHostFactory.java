...
public class HttpHostFactory {
    ...
    public HttpHostFactory(final Protocol httpProtocol, final Protocol httpsProtocol) {
    ...
    public HttpHost getHost(final HostConfiguration old, final String scheme, final String host, final int port) {
        return new HttpHost(host, port, getProtocol(old, scheme, host, port));
    }

    /**
     * Get a Protocol for the given parameters. The default implementation selects a protocol based only
     * on the scheme. Subclasses can do fancier things, such as select SSL parameters based on the host
     * or port. This method must not return null.
     */
    protected Protocol getProtocol(final HostConfiguration old, final String scheme, final String host, final int port) {
