 ...
 *         new AuthSSLProtocolSocketFactory(new URL("file:my.keystore"), "mypassword", new URL("file:my.truststore"), "mypassword"), 443);
 *
...
public class AuthSSLProtocolSocketFactory implements SecureProtocolSocketFactory {
     ...
     *                           that the same password is used to protect the key and the keystore
     *                           itself.
    ...
    private SSLContext createSSLContext() {
        try {
            ...
            if (keystoreUrl != null) {
                final KeyStore keystore = createKeyStore(keystoreUrl, keystorePassword);
                    ...
                    while (aliases.hasMoreElements()) {
                        ...
                        if (certs != null) {
                            ...
                            for (int c = 0; c < certs.length; c++) {
                                if (certs[c] instanceof final X509Certificate cert) {
            ...
            if (truststoreUrl != null) {
                final KeyStore keystore = createKeyStore(truststoreUrl, truststorePassword);
                if (LOG.isDebugEnabled()) {
                    final Enumeration<String> aliases = keystore.aliases();
                    while (aliases.hasMoreElements()) {
                        final String alias = aliases.nextElement();
                        ...
                        if (trustedcert != null && trustedcert instanceof final X509Certificate cert) {
                            LOG.debug("  Subject DN: " + cert.getSubjectX500Principal());
                            LOG.debug("  Signature Algorithm: " + cert.getSigAlgName());
                            LOG.debug("  Valid from: " + cert.getNotBefore());
                            LOG.debug("  Valid until: " + cert.getNotAfter());
