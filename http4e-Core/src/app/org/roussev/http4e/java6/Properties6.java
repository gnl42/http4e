...
package org.roussev.http4e.java6;
...
/**
 * The <code>Properties</code> class represents a persistent set of properties. The
 * <code>Properties</code> can be saved to a stream or loaded from a stream. Each key and its
 ...
 * <p>
 * Because <code>Properties</code> inherits from <code>Hashtable</code>, the <code>put</code> and
 * <code>putAll</code> methods can be applied to a <code>Properties</code> object. Their use is
 * strongly discouraged as they allow the caller to insert entries whose keys or values are not
 * <code>Strings</code>. The <code>setProperty</code> method should be used instead. If the
 * <code>store</code> or <code>save</code> method is called on a "compromised"
 * <code>Properties</code> object that contains a non-<code>String</code> key or value, the call
 * will fail. Similarly, the call to the <code>propertyNames</code> or <code>list</code> method will
 * fail if it is called on a "compromised" <code>Properties</code> object that contains a
 * non-<code>String</code> key.
 *
 * <p>
 ...
 *
 * <p>
 * The {@link #loadFromXML(InputStream)} and {@link #storeToXML(OutputStream, String, String)}
 * methods load and store properties in a simple XML format. By default the UTF-8 character encoding
 * is used, however a specific encoding may be specified if required. An XML properties document has
 ...
 * </pre>
 *
 * @see <a href="../../../technotes/tools/solaris/native2ascii.html">native2ascii tool for
 *      Solaris</a>
 * @see <a href="../../../technotes/tools/windows/native2ascii.html">native2ascii tool for
 *      Windows</a>
 *
 *      <p>
 *      This class is thread-safe: multiple threads can share a single <tt>Properties</tt> object
...
//TODO delete this file once migrated to java6
public class Properties6 extends Hashtable<Object, Object> {
    /**
     ...
     * @serial
     */
    protected Properties6 defaults;
     ...
     */
    public Properties6(final Properties6 defaults) {
        this.defaults = defaults;
    }

    /**
     * Calls the <tt>Hashtable</tt> method <code>put</code>. Provided for parallelism with the
     * <tt>getProperty</tt> method. Enforces use of strings for property keys and values. The value
     * returned is the result of the <tt>Hashtable</tt> call to <code>put</code>.
     *
     * @param key   the key to be placed into this property list.
     * @param value the value corresponding to <tt>key</tt>.
     * @return the previous value of the specified key in this property list, or <code>null</code> if it
     *         did not have one.
     * @see #getProperty
     * @since 1.2
     */
     ...
     * <p>
     * Properties are processed in terms of lines. There are two kinds of line, <i>natural lines</i> and
     * <i>logical lines</i>. A natural line is defined as a line of characters that is terminated either
     * by a set of line terminator characters (<code>\n</code> or <code>\r</code> or <code>\r\n</code>)
     * or by the end of the stream. A natural line may be either a blank line, a comment line, or hold
     * all or some of a key-element pair. A logical line holds all the data of a key-element pair, which
     * may be spread out across several adjacent natural lines by escaping the line terminator sequence
     * with a backslash character <code>\</code>. Note that a comment line cannot be extended in this
     * manner; every natural line that is a comment must have its own comment indicator, as described
     ...
     * character; comment lines are also ignored and do not encode key-element information. In addition
     * to line terminators, this format considers the characters space (<code>' '</code>,
     ...
     * no remaining characters, the element is the empty string <code>&quot;&quot;</code>. Once the raw
     ...
     * </pre>
     *
     * Note that a space appears before each <code>\</code> so that a space will appear after each comma
     * in the final result; the <code>\</code>, line terminator, and leading white space on the
     ...
     * sequences similar to those used for character and string literals (see
     * <a href="http://java.sun.com/docs/books/jls/third_edition/html/lexical.html#3.3">&sect;3.3</a>
     * and <a href=
     * "http://java.sun.com/docs/books/jls/third_edition/html/lexical.html#3.10.6">&sect;3.10.6</a> of
    ...
    /**
     * Reads a property list (key and element pairs) from the input byte stream. The input stream is in
     * a simple line-oriented format as specified in {@link #load(java.io.Reader) load(Reader)} and is
     * assumed to use the ISO 8859-1 character encoding; that is each byte is one Latin1 character.
     * Characters not in Latin1, and certain special characters, are represented in keys and elements
     * using <a href="http://java.sun.com/docs/books/jls/third_edition/html/lexical.html#3.3">Unicode
     * escapes</a>.
     * <p>
     * The specified stream remains open after this method returns.
     *
    ...
    /*
     * Read in a "logical line" from an InputStream/Reader, skip all comment and blank lines and filter
     * out those leading whitespace characters (\u0020, \u0009 and \u000c) from the beginning of a
    ...
    /**
     * Calls the <code>store(OutputStream out, String comments)</code> method and suppresses
     * IOExceptions that were thrown.
     *
     * @deprecated This method does not throw an IOException if an I/O error occurs while saving the
     *             property list. The preferred way to save a properties list is via the
     *             <code>store(OutputStream out,
     * String comments)</code> method or the <code>storeToXML(OutputStream os, String comment)</code>
     *             method.
     *
     * @param out      an output stream.
     * @param comments a description of the property list.
     * @exception ClassCastException if this <code>Properties</code> object contains any keys or values
     ...
     * load(Reader)} method.
     * <p>
     * Properties from the defaults table of this <code>Properties</code> table (if any) are <i>not</i>
     ...
     * comments is not character <code>#</code> or character <code>!</code> then an ASCII <code>#</code>
     ...
     * string. For the key, all space characters are written with a preceding <code>\</code> character.
     * For the element, leading space characters, but not embedded or trailing space characters, are
     * written with a preceding <code>\</code> character. The key and element characters <code>#</code>,
     * <code>!</code>, <code>=</code>, and <code>:</code> are written with a preceding backslash to
     * ensure that they are properly loaded.
     * <p>
     * After the entries have been written, the output stream is flushed. The output stream remains open
     * after this method returns.
     * <p>
     *
     * @param writer   an output character stream writer.
     * @param comments a description of the property list.
     * @exception IOException          if writing this property list to the specified output stream
     *                                 throws an <tt>IOException</tt>.
     * @exception ClassCastException   if this <code>Properties</code> object contains any keys or
     ...
     * <code>&#92;u007E</code> in property keys or values are written as <code>&#92;u</code><i>xxxx</i>
     * for the appropriate hexadecimal value <i>xxxx</i>.
     * </ul>
     * <p>
     * After the entries have been written, the output stream is flushed. The output stream remains open
     * after this method returns.
    ...
    /**
     * Loads all of the properties represented by the XML document on the specified input stream into
     ...
     *                                          <tt>IOException</tt>.
     * @throws InvalidPropertiesFormatException Data on input stream does not constitute a valid XML
...
//        in.close();
     ...
     * @throws ClassCastException   if this <code>Properties</code> object contains any keys or values
     *                              that are not <code>Strings</code>.
     ...
     *
     * <p>
     * If the specified comment is <code>null</code> then no comment will be stored in the document.
     *
     * <p>
     * The specified stream remains open after this method returns.
     *
     ...
     * @throws IOException          if writing to the specified output stream results in an
     *                              <tt>IOException</tt>.
     * @throws NullPointerException if <code>os</code> is <code>null</code>, or if <code>encoding</code>
     *                              is <code>null</code>.
     * @throws ClassCastException   if this <code>Properties</code> object contains any keys or values
     *                              that are not <code>Strings</code>.
     * @see #loadFromXML(InputStream)
...
//        XMLUtils.save(this, os, comment, encoding);
     ...
     * properties list.
     *
     * @return an enumeration of all the keys in this property list, including the keys in the default
     *         property list.
     ...
     */
    /*
     * Rather than use an anonymous inner class to share common code, this method is duplicated in order
     * to ensure that a non-1.1 compiler can compile this file.
     */
    ...
    /**
     * Enumerates all key/value pairs in the specified hashtable and omits the property if the key or
