 ...
 * </dl>
 * <p>
 * Note: Only one of SHADOW_IN, SHADOW_OUT and SHADOW_NONE may be specified. If neither ist
 * specified, the default value SHADOW_IN is used. If SHADOW_NONE is specified, a single line is
 * drawn with the control's foreground color. Only one of HORIZONTAL and VERTICAL may be specified.
...
public final class CustomSeparator extends Canvas {
    ...
    public CustomSeparator(final Composite parent, int style) {
    ...
    private static int checkStyle(int style) {
        final int mask = SWT.SHADOW_IN | SWT.SHADOW_OUT | SWT.SHADOW_NONE | SWT.HORIZONTAL | SWT.VERTICAL;
        style &= mask;
    ...
    private void onPaint(final PaintEvent event) {
        ...
        final boolean horiz = (style & SWT.HORIZONTAL) != 0;
        final int mid = horiz ? r.y + r.height / 2 : r.x + r.width / 2;
