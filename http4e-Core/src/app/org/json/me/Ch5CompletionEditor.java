...
public class Ch5CompletionEditor extends Composite {
    ...
    public Ch5CompletionEditor(final Composite parent) {
...
class WordTracker {
    ...
    private final Map<String, String> knownWords = new HashMap<>();
    ...
    private void flushOldestWord() {
        if (wordBuffer.size() == maxQueueSize) {
            final String removedWord = wordBuffer.remove(maxQueueSize - 1);
    ...
    public List<String> suggest(final String word) {
        ...
        for (final Iterator<String> i = wordBuffer.iterator(); i.hasNext();) {
            final String currWord = i.next();
...
class RecentWordContentAssistProcessor implements IContentAssistProcessor {
    ...
    public ICompletionProposal[] computeCompletionProposals(final ITextViewer textViewer, final int documentOffset) {
        ...
        try {
            ...
            final List<String> suggestions = wordTracker.suggest(currWord);
    ...
    private ICompletionProposal[] buildProposals(final List<String> suggestions, final String replacedWord, final int offset) {
        final ICompletionProposal[] proposals = new ICompletionProposal[suggestions.size()];
        int index = 0;
    ...
    @Override
    public IContextInformation[] computeContextInformation(final ITextViewer textViewer, final int documentOffset) {
